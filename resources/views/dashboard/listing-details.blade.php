@include('header');
<link rel="stylesheet" href="{{url('/assets/dashboard/css/listing.css')}}" />

<body class="listing-body">
<div class="listingHolder">
    <div class="gallery">
        @foreach($data['listingInfo']['image'] as $images)
            <img src="{{url($images['path'])}}" alt="a forest after an apocalypse">
...
{{--            <img src="https://picsum.photos/id/1057/300/300" alt="a great view of the sea above the mountain">--}}
    ...
    <div class="detailsListing">
        <h2>{{$data['listingInfo']['title']}}</h2>
        <p>{{$data['listingInfo']['description']}}</p>
    </div>
</div>
</body>
